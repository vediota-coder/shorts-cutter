...
def pick_clips_with_usage(
    ...
    *,
    ...
    # max_clips <= 0 → AI-auto: LLM сам решает оптимальное число клипов.
    auto_mode = max_clips is None or max_clips <= 0
    if auto_mode:
        user_msg = (
            f"Транскрипт (формат [mm:ss] текст):\n\n{transcript}\n\n"
            "АВТО-РЕЖИМ: реши САМ, сколько клипов оптимально из этого видео. "
            "НЕ натягивай слабые моменты до круглого числа — лучше меньше, но сильнее. "
    ...
    # ⭐ AI-auto: добавляем блок-инструкцию, чтобы LLM ориентировался по длине
    # транскрипта и реальной плотности wow-моментов, а не по фиксированному числу.
    if auto_mode:
        SYSTEM = (
            SYSTEM.rstrip()
            + "\n\n=== АВТО-РЕЖИМ ВЫБОРА КОЛИЧЕСТВА ===\n"
            + "Количество клипов решаешь САМ — пользователь не задавал лимит.\n"
            + "Ориентир по длительности исходника:\n"
            + "  • короткое (<10 мин) — 0–3 клипа;\n"
            + "  • среднее (10–40 мин) — 3–7 клипов;\n"
            + "  • длинное (>40 мин) — 7–15 клипов.\n"
            + "Это ориентиры, а не квоты: главный критерий — реальная плотность "
            + "wow-моментов. Если в часовом видео есть только 2 по-настоящему сильных "
            + "фрагмента — верни 2. Если в 15-минутном плотно идёт 8 — верни 8. "
            + "НИКОГДА не добивай список слабыми фрагментами ради круглого числа.\n"
            + "=== КОНЕЦ АВТО-РЕЖИМА ===\n"
        )
